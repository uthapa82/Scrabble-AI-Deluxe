...
public class Scorer {
	...
	private void addWordHor(Space newLetter) {
		...
		wordSpaces.pushBack(new Vector<Space>());
	...
	private boolean tileIsAnchored(Space space) {
		...
		if (row + 1 < 15 && board.getSpace(row + 1, col).getTile() != null
			|| row - 1 >= 0 && board.getSpace(row - 1, col).getTile() != null
			|| col + 1 < 15 && board.getSpace(row, col + 1).getTile() != null
			|| col - 1 >= 0 &&board.getSpace(row, col - 1).getTile() != null) {
			return true;
		} else {
			return false;
		}
	...
	public void reset() {
		...
		matchedWords = new Vector<String>();
		score = 0;
	}
